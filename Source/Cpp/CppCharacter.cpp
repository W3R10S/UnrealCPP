...
{
	// If Cpp is added to a project via 'Add Feature' in the Unreal Editor the dependency on HeadMountedDisplay in Cpp.Build.cs is not automatically propagated
...
void ACppCharacter::OnHealthChanged() {
	if (Health <= 0) {
		Death();
		GetWorldTimerManager().SetTimer(TimerHandle, this, &ACppCharacter::Respawn, DeathDuration, false, DeathDuration);
		//Respawn();
	}
}

void ACppCharacter::Death() {
	GetMesh()->SetHiddenInGame(true, false);
	DisableInput(NULL);
	SetActorTickEnabled(false);
}

void ACppCharacter::Respawn() {
	TeleportTo(SpawnLocation->GetActorLocation(), SpawnLocation->GetActorRotation());
